...
def make_builder(online: bool = True, public_only: bool = False) -> DayBuilder:
    ...
    met = poetry = wiki = reflection = None
    if online:
        ...
        met = MetSource(image_dir=images)
        wiki = WikipediaClient()
        reflection = ReflectionWriter()
        taste_gate = reflection.taste_ok if os.environ.get("ANTHROPIC_API_KEY") else None
        poetry = PoetrySource(taste_gate=taste_gate)
    return DayBuilder(
        store=store, library=library, met=met, poetry=poetry,
